...
describe Decidim::Initiatives::Permissions do
  ...
  context "when reading an initiative" do
    ...
    context "when initiative is illegal" do
      let(:initiative) { create(:initiative, state: :illegal, organization: organization) }

      it { is_expected.to be true }
    end

    context "when initiative is invalidated" do
      let(:initiative) { create(:initiative, state: :invalidated, organization: organization) }

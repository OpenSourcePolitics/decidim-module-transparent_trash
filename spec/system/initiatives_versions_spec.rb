...
describe "Explore versions", versioning: true, type: :system do
  ...
  context "when initiative is illegal" do
    let(:initiative) { create(:initiative, :illegal, organization: organization) }
    ...
    context "when showing version" do
      ...
      context "and user is not logged in" do
        ...
        it "can't access to versioning" do
          expect(page).to have_content("You are not authorized")
        end
      end

      context "and user is logged in" do
        context "and has no authorship" do
          let(:not_author) { create(:user, :confirmed, organization: organization) }

          before do
            relogin_as not_author
            click_link "see other versions"
          end
        ...
        context "and has authorship" do
          before do
            relogin_as initiative.author
            click_link "see other versions"
          end

          it "can access to versioning" do
